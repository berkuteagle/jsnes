...
import ROM from 'rom';
import MappersManager from 'mappers';
...
export default class JSNES {
    ...
    loadRom(data) {
        ...
        if (this.rom.valid) {
            ...
            let mapperId = this.rom.mapperType;
            let mapper = MappersManager.getMapper(mapperId);
            if (mapper) {
                this.mmap = new mapper(this);
                if (!this.mmap) {
                    return;
                }
            } else {
                this.ui.updateStatus(`This ROM uses a mapper not supported by JSNES: ${MappersManager.getName(mapperId)} (${mapperId})`);
            }
